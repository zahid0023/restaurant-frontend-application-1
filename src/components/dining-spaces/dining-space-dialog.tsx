...
import type {
  DiningSpaceDialogMode,
  DiningSpaceFormState,
  DiningSpaceLocaleRow,
} from "./types";
...
export interface DiningSpaceDialogProps {
  ...
  onModeChange?: (mode: DiningSpaceDialogMode) => void;
  spaceId?: number;
...
export function DiningSpaceDialog({
  ...
  const originalLocalesRef = useRef<Map<number, DiningSpaceLocaleRow>>(new Map());
  useEffect(() => {
    if (open) {
      const map = new Map<number, DiningSpaceLocaleRow>();
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!form.dining_space_type_id) {
      toast.error(t("diningSpace.errType"));
      return;
    }
    if (!form.code.trim()) {
      toast.error(t("diningSpace.errCode"));
      return;
    }

    for (const [i, row] of form.locales.entries()) {
      if (!row.locale_id) {
        toast.error(t("diningSpace.errLocaleLang", { n: i + 1 }));
        return;
      }
      if (!row.name.trim()) {
        toast.error(t("diningSpace.errLocaleName", { n: i + 1 }));
        return;
      }
    }
    ...
    try {
      ...
      if (mode === "edit" && spaceId != null) {
        await diningSpacesService.update(spaceId, base);

        for (const id of removedIds) {
          await diningSpacesService.removeLocale(spaceId, id);
        }
        ...
        toast.success(
          parts.length
            ? `${t("diningSpace.updatedToast")} ${base.code} · locales: ${parts.join(", ")}`
            : `${t("diningSpace.updatedToast")} ${base.code}`,
        );
      } else {
        ...
        toast.success(`${t("diningSpace.createdToast")} ${base.code}`);
      }
      onOpenChange(false);
      await onSaved?.();
    } catch (err) {
  ...
  const selectedType = availableTypes.find((t) => t.id === Number(form.dining_space_type_id));
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <DialogHeader>
            <DialogTitle>
              ...
              {mode === "edit" && t("diningSpace.titleEdit")}
              {mode === "view" && t("diningSpace.titleView")}
            </DialogTitle>
            <DialogDescription>
              {mode === "create" && t("diningSpace.descCreate")}
              {mode === "edit" && t("diningSpace.descEdit")}
              {mode === "view" && t("diningSpace.descView")}
            </DialogDescription>
          </DialogHeader>

          {/* Type */}
          <div className="space-y-2">
            <Label htmlFor="type_id">{t("diningSpace.type")} *</Label>
            {readOnly || mode === "edit" ? (
              <Input
                id="type_id"
                value={selectedType ? selectedType.code : String(form.dining_space_type_id)}
                disabled
              />
            ) : (
              <Select
          ...
          {/* Floor */}
          <div className="space-y-2">
            <Label htmlFor="floor_id">{t("diningSpace.floor")}</Label>
            {readOnly ? (
              <Input
                id="floor_id"
                value={
                  form.floor_id != null
                    ? (selectedFloor ? selectedFloor.code : String(form.floor_id))
                    : "—"
                }
                disabled
              />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div className="space-y-2">
              <Label htmlFor="capacity">{t("diningSpace.capacity")} *</Label>
              <Input
                id="capacity"
                type="number"
                min={1}
                value={form.capacity}
                onChange={(e) => setForm({ capacity: Number(e.target.value) })}
                required
                disabled={readOnly}
              />
            </div>
            <div className="space-y-2">
              <Label>{t("diningSpace.isBookable")}</Label>
              <label className="flex items-center gap-2 h-10 cursor-pointer">
                <input
                  type="checkbox"
                  checked={form.is_bookable}
                  onChange={(e) => setForm({ is_bookable: e.target.checked })}
                  disabled={readOnly}
                  className="h-4 w-4 rounded border-input accent-primary"
                />
                <span className="text-sm">
                  {form.is_bookable ? t("diningSpace.bookable") : t("diningSpace.notBookable")}
                </span>
              </label>
            </div>
          </div>
          ...
          <div className="space-y-3 border-t pt-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  ...
                  {mode === "view" && t("diningSpace.localeHelpView")}
                </p>
              </div>
            ...
            {form.locales.length === 0 ? (
              <p className="text-sm text-muted-foreground text-center py-6 border rounded-lg border-dashed">
                {mode === "create" ? t("locale.empty.create") : t("diningSpace.noLocales")}
              </p>
            ) : (

...
import { Check, Languages, Pencil, Plus, Tag, Trash2, X } from "lucide-react";
...
export function ItemTypeDialog({
  ...
  async function saveGeneral() {
    ...
    try {
      ...
      setGeneralEditing(false);
      onFormChange({ ...form, is_consumable: localGeneral.is_consumable, sort_order: Number(localGeneral.sort_order) || 0 });
      await onSaved?.();
    } catch (err) {
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <DialogTitle className="text-base font-semibold leading-tight">
                  ...
                  {mode !== "create" && (generalEditing || translationsEditing ? t("itemType.titleEdit") : t("itemType.titleView"))}
                </DialogTitle>
                <DialogDescription className="text-xs mt-0.5">
                  {mode === "create" && t("itemType.descCreate")}
                  {mode !== "create" && (generalEditing || translationsEditing ? t("itemType.descEdit") : t("itemType.descView"))}
                </DialogDescription>
              </div>
            </div>
          </DialogHeader>
              ...
              <div className="rounded-xl border bg-card p-4 space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="space-y-2">
                    ...
                    <Select
                      value={generalEditing ? String(localGeneral.is_consumable) : String(form.is_consumable)}
                      onValueChange={(v) => setLocalGeneral({ ...localGeneral, is_consumable: v === "true" })}
                      disabled={!generalEditing && mode !== "create"}
                    >
                      <SelectTrigger id="it-consumable">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>

...
export interface ItemTypeDialogProps {
  ...
  onModeChange?: (mode: ItemTypeDialogMode) => void;
  typeId?: number;
...
export function ItemTypeDialog({
  ...
  const originalLocalesRef = useRef<Map<number, ItemTypeLocaleRow>>(new Map());
  useEffect(() => {
    if (open) {
      const map = new Map<number, ItemTypeLocaleRow>();
  ...
  function setForm(patch: Partial<ItemTypeFormState>) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (readOnly) return;
    if (!form.code.trim()) {
      toast.error(t("itemType.errCode"));
      return;
    }

    for (const [i, row] of form.locales.entries()) {
      if (!row.locale_id) {
        toast.error(t("itemType.errLocaleLang", { n: i + 1 }));
        return;
      }
      if (!row.name.trim()) {
        toast.error(t("itemType.errLocaleName", { n: i + 1 }));
        return;
      }
    }
    ...
    try {
      ...
      if (mode === "edit" && typeId != null) {
        await itemTypesService.update(typeId, base);

        for (const id of removedIds) {
          await itemTypesService.removeLocale(typeId, id);
        }
        ...
        toast.success(
          parts.length
            ? `${t("itemType.updatedToast")} ${base.code} · locales: ${parts.join(", ")}`
            : `${t("itemType.updatedToast")} ${base.code}`,
        );
      } else {
        ...
        toast.success(`${t("itemType.createdToast")} ${base.code}`);
      }
      onOpenChange(false);
      await onSaved?.();
    } catch (err) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <form onSubmit={handleSubmit} className="space-y-5">
          <DialogHeader>
            <DialogTitle>
              {mode === "create" && t("itemType.titleCreate")}
              {mode === "edit" && t("itemType.titleEdit")}
              {mode === "view" && t("itemType.titleView")}
            </DialogTitle>
            <DialogDescription>
              {mode === "create" && t("itemType.descCreate")}
              {mode === "edit" && t("itemType.descEdit")}
              {mode === "view" && t("itemType.descView")}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div className="space-y-2">
              <Label>{t("itemType.isConsumable")}</Label>
              <label className="flex items-center gap-2 h-10 cursor-pointer">
                <input
                  type="checkbox"
                  checked={form.is_consumable}
                  onChange={(e) => setForm({ is_consumable: e.target.checked })}
                  disabled={readOnly}
                  className="h-4 w-4 rounded border-input accent-primary"
                />
                <span className="text-sm">
                  {form.is_consumable ? t("itemType.consumable") : t("itemType.nonConsumable")}
                </span>
              </label>
            </div>
          </div>
          ...
          <div className="space-y-3 border-t pt-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  ...
                  {mode === "view" && t("itemType.localeHelpView")}
                </p>
              </div>
            ...
            {form.locales.length === 0 ? (
              <p className="text-sm text-muted-foreground text-center py-6 border rounded-lg border-dashed">
                {mode === "create" ? t("locale.empty.create") : t("itemType.noLocales")}
              </p>
            ) : (
                    ...
                    <div key={idx} className="rounded-lg border p-3 space-y-3">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <Input
                          ...
                          placeholder={t("itemType.namePlaceholder")}
                          disabled={fieldsReadOnly}
                        />
                      </div>
                      ...
                      <div className="space-y-1.5">
                        ...
                        <Textarea
                          ...
                          placeholder={t("itemType.descriptionPlaceholder")}
                          disabled={fieldsReadOnly}
                          rows={2}
                        />
                      </div>

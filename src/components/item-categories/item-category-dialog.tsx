...
export function ItemCategoryDialog({
  ...
  const originalLocalesRef = useRef<Map<number, ItemCategoryLocaleRow>>(new Map());
  useEffect(() => {
    if (open) {
      const map = new Map<number, ItemCategoryLocaleRow>();
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    for (const [i, row] of form.locales.entries()) {
      if (!row.locale_id) {
        toast.error(t("itemCategory.errLocaleLang", { n: i + 1 }));
        return;
      }
      if (!row.name.trim()) {
        toast.error(t("itemCategory.errLocaleName", { n: i + 1 }));
        return;
      }
    }
    ...
    try {
      ...
      if (mode === "edit" && categoryId != null) {
        ...
        for (const id of removedIds) {
          await itemCategoriesService.removeLocale(itemTypeId, categoryId, id);
        }
        ...
        toast.success(
          parts.length
            ? `${t("itemCategory.updatedToast")} ${code} · locales: ${parts.join(", ")}`
            : `${t("itemCategory.updatedToast")} ${code}`,
        );
      } else {
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <DialogHeader>
            <DialogTitle>
              ...
              {mode === "edit" && t("itemCategory.titleEdit")}
              {mode === "view" && t("itemCategory.titleView")}
            </DialogTitle>
            <DialogDescription>
              {mode === "create" && t("itemCategory.dialogDesc")}
              {mode === "edit" && t("itemCategory.dialogDescEdit")}
              {mode === "view" && t("itemCategory.dialogDescView")}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-3 border-t pt-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  ...
                  {mode === "edit" && t("itemCategory.localeHintEdit")}
                  {mode === "view" && t("itemCategory.localeHintView")}
                </p>
              </div>
              {(mode === "create" || mode === "edit") && (
                ...
                >
                  <Plus className="h-4 w-4 mr-1" /> {t("itemCategory.addLocale")}
                </Button>
              )}
            </div>
            ...
            {form.locales.length === 0 ? (
              <p className="text-sm text-muted-foreground text-center py-6 border rounded-lg border-dashed">
                {mode === "create" ? t("itemCategory.noLocalesCreate") : t("itemCategory.noLocalesView")}
              </p>
            ) : (

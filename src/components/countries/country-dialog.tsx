...
export function CountryDialog({
  ...
  const originalLocalesRef = useRef<Map<number, LocaleRow>>(new Map());
  useEffect(() => {
    if (open) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (mode === "edit" && countryId != null) {
        await countriesService.update(countryId, base);

        for (const id of removedIds) {
          await countriesService.removeLocale(countryId, id);
        }
        ...
        toast.success(
          parts.length
            ? `Updated ${base.code} · locales: ${parts.join(", ")}`
            : `Updated ${base.code}`,
        );
      } else {
        ...
        toast.success(`Created ${base.code}`);
      }
      onOpenChange(false);
      await onSaved?.();
    } catch (err) {
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <DialogHeader>
            <DialogTitle>
              ...
              {mode === "edit" && t("dialog.country.edit")}
              {mode === "view" && t("dialog.country.view")}
            </DialogTitle>
            <DialogDescription>
              {mode === "create" && t("dialog.country.desc.create")}
              {mode === "edit" && t("dialog.country.desc.edit")}
              {mode === "view" && t("dialog.country.desc.view")}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="space-y-2">
              <Label htmlFor="iso3">{t("field.iso3")}</Label>
              <Input
                id="iso3"
                value={form.iso3_code}
                onChange={(e) => setForm({ iso3_code: e.target.value })}
                placeholder="BGD"
                disabled={readOnly}
              />
            </div>
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setForm({ phone_code: e.target.value })}
                placeholder="+880"
                disabled={readOnly}
              />
            </div>
          ...
          <div className="space-y-3 border-t pt-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  ...
                  {mode === "view" && t("locale.help.view.country")}
                </p>
              </div>
            ...
            {form.locales.length === 0 ? (
              <p className="text-sm text-muted-foreground text-center py-6 border rounded-lg border-dashed">
                {mode === "create" ? t("locale.empty.create") : t("locale.empty.country")}
              </p>
            ) : (

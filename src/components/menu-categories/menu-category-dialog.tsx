...
export interface MenuCategoryDialogProps {
  ...
  onModeChange?: (mode: MenuCategoryDialogMode) => void;
  categoryId?: number;
  menuId?: number;
...
export function MenuCategoryDialog({
  ...
  const originalLocalesRef = useRef<Map<number, MenuCategoryLocaleRow>>(new Map());
  useEffect(() => {
    if (open) {
      const map = new Map<number, MenuCategoryLocaleRow>();
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (readOnly) return;
    if (!form.code.trim()) {
      toast.error(t("menuCategory.errCode"));
      return;
    }
    for (const [i, row] of form.locales.entries()) {
      if (!row.locale_id) {
        toast.error(t("menuCategory.errLocaleLang", { n: i + 1 }));
        return;
      }
      if (!row.name.trim()) {
        toast.error(t("menuCategory.errLocaleName", { n: i + 1 }));
        return;
      }
    }
    ...
    try {
      ...
      if (mode === "edit" && categoryId != null && menuId != null) {
        await menuCategoriesService.update(categoryId, base);

        for (const id of removedIds) {
          await menuCategoriesService.removeLocale(menuId, categoryId, id);
        }
        ...
        for (const row of updatedRows) {
          await menuCategoriesService.updateLocale(menuId, categoryId, row.id!, {
        ...
        for (const row of newRows) {
          await menuCategoriesService.addLocale(menuId, categoryId, {
            locale_id: Number(row.locale_id),
            name: row.name.trim(),
            description: row.description.trim() || undefined,
            sort_order: Number(row.sort_order) || 0,
        ...
        toast.success(
          parts.length
            ? `${t("menuCategory.updatedToast")} ${base.code} · locales: ${parts.join(", ")}`
            : `${t("menuCategory.updatedToast")} ${base.code}`,
        );
      } else {
        ...
        toast.success(`${t("menuCategory.createdToast")} ${base.code}`);
      }
      onOpenChange(false);
      await onSaved?.();
    } catch (err) {
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <DialogHeader>
            <DialogTitle>
              ...
              {mode === "edit" && t("menuCategory.titleEdit")}
              {mode === "view" && t("menuCategory.titleView")}
            </DialogTitle>
            <DialogDescription>
              {mode === "create" && t("menuCategory.descCreate")}
              {mode === "edit" && t("menuCategory.descEdit")}
              {mode === "view" && t("menuCategory.descView")}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-3 border-t pt-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  ...
                  {mode === "view" && t("menuCategory.localeHelpView")}
                </p>
              </div>
            ...
            {form.locales.length === 0 ? (
              <p className="text-sm text-muted-foreground text-center py-6 border rounded-lg border-dashed">
                {mode === "create" ? t("locale.empty.create") : t("menuCategory.noLocales")}
              </p>
            ) : (

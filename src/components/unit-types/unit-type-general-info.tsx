...
export interface UnitTypeGeneralInfoProps {
  mode: UnitTypeDialogMode;
  form: UnitTypeFormState;
  onFormChange: (patch: Partial<UnitTypeFormState>) => void;
  unitTypeId?: number;
...
export function UnitTypeGeneralInfo({
  mode,
  form,
  onFormChange,
  unitTypeId,
  ...
  async function save() {
    ...
    setSubmitting(true);
    try {
      await unitTypesService.update(unitTypeId, { sort_order: Number(localSortOrder) || 0 });

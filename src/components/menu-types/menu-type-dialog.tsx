...
export interface MenuTypeDialogProps {
  ...
  onModeChange?: (mode: MenuTypeDialogMode) => void;
  menuTypeId?: number;
...
export function MenuTypeDialog({
  ...
  const originalLocalesRef = useRef<Map<number, MenuTypeLocaleRow>>(new Map());
  useEffect(() => {
    if (open) {
      const map = new Map<number, MenuTypeLocaleRow>();
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (readOnly) return;
    if (!form.code.trim()) {
      toast.error(t("menuType.errCode"));
      return;
    }
    for (const [i, row] of form.locales.entries()) {
      if (!row.locale_id) {
        toast.error(t("menuType.errLocaleLang", { n: i + 1 }));
        return;
      }
      if (!row.name.trim()) {
        toast.error(t("menuType.errLocaleName", { n: i + 1 }));
        return;
      }
    }
    ...
    try {
      ...
      if (mode === "edit" && menuTypeId != null) {
        await menusService.update(menuTypeId, base);

        for (const id of removedIds) {
          await menusService.removeLocale(menuTypeId, id);
        }
        ...
        toast.success(
          parts.length
            ? `${t("menuType.updatedToast")} ${base.code} · locales: ${parts.join(", ")}`
            : `${t("menuType.updatedToast")} ${base.code}`,
        );
      } else {
        ...
        toast.success(`${t("menuType.createdToast")} ${base.code}`);
      }
      onOpenChange(false);
      await onSaved?.();
    } catch (err) {
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <DialogHeader>
            <DialogTitle>
              ...
              {mode === "edit" && t("menuType.titleEdit")}
              {mode === "view" && t("menuType.titleView")}
            </DialogTitle>
            <DialogDescription>
              {mode === "create" && t("menuType.descCreate")}
              {mode === "edit" && t("menuType.descEdit")}
              {mode === "view" && t("menuType.descView")}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-3 border-t pt-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  ...
                  {mode === "view" && t("menuType.localeHelpView")}
                </p>
              </div>
            ...
            {form.locales.length === 0 ? (
              <p className="text-sm text-muted-foreground text-center py-6 border rounded-lg border-dashed">
                {mode === "create" ? t("locale.empty.create") : t("menuType.noLocales")}
              </p>
            ) : (

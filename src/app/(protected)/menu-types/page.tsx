...
import { MenuTypeCard } from "@/components/menu-types/menu-type-card";
...
export default function MenuTypesPage() {
  ...
  const [menuTypes, setMenuTypes] = useState<Menu[]>([]);
  ...
  const [mode, setMode] = useState<MenuTypeDialogMode>("create");
  const [activeId, setActiveId] = useState<number | undefined>(undefined);
  const [form, setForm] = useState<MenuTypeFormState>(emptyMenuTypeForm);
  ...
  async function refresh() {
    setLoading(true);
    try {
      const res = await menusService.list({ size: 50, sort_by: "sortOrder" });
  ...
  const menuTypeNames = useMemo(() => {
    const out: Record<number, string> = {};
    for (const m of menuTypes) {
      out[m.id] = m.locales?.[0]?.name ?? "";
    }
    return out;
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [menuTypes, locale]);
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return menuTypes;
    return menuTypes.filter((m) => {
      const code = m.code.toLowerCase();
      const name = (menuTypeNames[m.id] ?? "").toLowerCase();
      switch (searchField) {
  ...
  }, [menuTypes, menuTypeNames, search, searchField]);

  function openCreate() {
    setMode("create");
    setActiveId(undefined);
    setForm(emptyMenuTypeForm);
    setDialogOpen(true);
  }
  ...
  async function confirmDelete() {
    ...
    try {
      await menusService.remove(deleteTarget.id);
      toast.success(`${t("menuType.deletedToast")} ${deleteTarget.code}`);
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          ...
          <h1 className="text-3xl font-semibold tracking-tight">{t("menuType.pageTitle")}</h1>
          <p className="text-muted-foreground text-sm mt-1">{t("menuType.pageSubtitle")}</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button onClick={openCreate}>
            <Plus className="h-4 w-4 mr-1.5" /> {t("menuType.new")}
          </Button>
        </div>
      </div>

      {loading ? (
        <div className="text-center py-16 text-muted-foreground">{t("menuType.loading")}</div>
      ) : filtered.length === 0 ? (
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(o) => !o && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("menuType.deleteTitle")}</AlertDialogTitle>
            <AlertDialogDescription>
              {t("menuType.deleteDesc", { code: deleteTarget?.code ?? "" })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
